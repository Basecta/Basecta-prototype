...
from app.schemas.user import UserCreate, UserLogin, UserResponse, TokenResponse, PasswordChange, GoogleLogin, GoogleTokenResponse
...
def change_password(
    ...
    # If the user has a password, verify the current one before allowing a change
...
def google_login(data: GoogleLogin, db: Session = Depends(get_db)):
    ...
    if is_new_user:
        # Derive a unique username from the Google display name or email
